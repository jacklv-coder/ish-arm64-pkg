...
当前默认分支已发布 `v0.4.0-abi.9`，正在准备兼容性维护预发布
`v0.4.0-abi.10`。它们都属于 **Stage1 native ABI 过渡**，不是稳定 `v0.4.0`，
也不是完整 v0.4 Swift API。请同时区分下面四个版本面：

| 版本面 | 当前 `v0.4.0-abi.9` | 计划中的 `v0.4.0-abi.10` |
| --- | --- | --- |
| 公开 C ABI | `ISH_EMBED_ABI_VERSION == 1`；原子 rename、有限 stdin deadline 与单次 stdin timeout API 已发布 | 仍为 ABI 1；只更新内部 task/address-space 生命周期 |
| 内部 wire protocol | host 与内嵌 supervisor 精确匹配 v4 | 仍为 v4；它不是公开 C ABI 版本 |
| `Package.swift` | 固定已公开的 `v0.4.0-abi.9` URL/checksum | 发布事务生成只改 manifest 的 release commit，固定到维护二进制 |
...
当前 `v0.4.0-abi.10` 源码变更不纳入 RootFS，也不提交任何预构建
XCFramework/guest binary；二进制只能在后续发布事务通过后生成和发布。

## 安装状态

`v0.4.0-abi.9` 已公开且当前 [`Package.swift`](Package.swift) 固定到它。
`v0.4.0-abi.10` 发布前，manifest 继续指向这个已验证的资产，不会提前引用 404 URL。
...
`v0.4.0-abi.9` 已提供无 shell 的 guest 原子重命名、有界 stdin deadline 和单次
write/close timeout。`v0.4.0-abi.10` 修复强制 guest task teardown 与 interval timer、
共享 fd、阻塞 syscall 和 native output worker 之间的资源生命周期竞态。它不实现原生
Agent Loop，也不会
