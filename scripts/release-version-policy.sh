...
本维护版本还会确保嵌入启动线程及 guest task 线程解除内部 SIGUSR1 屏蔽，
使 guest signal 能可靠打断阻塞中的宿主 syscall。
...
wire protocol v4. This maintenance release also unblocks iSH's internal
SIGUSR1 on embedded bootstrap and guest task threads, allowing guest signals
to reliably interrupt blocking host syscalls. The public C ABI remains
