...
ish_release_stage1_version_allowed v0.4.0-abi.14 || {
    printf 'error: authorized Stage1 maintenance tag was rejected\n' >&2
    exit 1
}
for forbidden in v0.4.0 v0.4.0-abi.1 v0.4.0-abi.2 v0.4.0-abi.3 v0.4.0-abi.4 v0.4.0-abi.5 v0.4.0-abi.6 v0.4.0-abi.7 v0.4.0-abi.8 v0.4.0-abi.9 v0.4.0-abi.10 v0.4.0-abi.11 v0.4.0-abi.12 v0.4.0-abi.13 v0.4.0-rc.1 v1.2.3; do
...
for expected in \
    ...
    'later readers cannot barge' \
    'waitpid(WNOHANG)' \
    'cleanup deadline' \
    'fail-closing the VM' \
    'AdvSIMD `REV16`' \
    '清零' \
    'Rust/TLS network paths' \
    'guest exits with `SIGILL`' \
    'Linux socket 地址布局转换' \
    '`sa_len`/`sa_family`' \
    '`bind`、`connect` 与 `sendto`' \
    'Linux socket-address translation' \
    '`bind`, `connect`, and `sendto`' \
    'misread Linux family bytes' \
