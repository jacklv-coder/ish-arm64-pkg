...
3. **Swift source and manifest binary**: Stage1 Swift remains v0.3.3-ABI
   ...
   public `v0.4.0-abi.6`; the release transaction moves it to the
   `v0.4.0-abi.7` maintenance binary only after those assets are public and
   verified.
...
`v0.4.0-abi.6` and its `v0.4.0-abi.7` maintenance release are native-first
...
| [Release transaction](releasing.en.md) | Why does the merged maintenance source still pin `v0.4.0-abi.6`, and when does it become `v0.4.0-abi.7`? |
...
- The current `v0.4.0-abi.7` source change carries no RootFS or prebuilt binary;
