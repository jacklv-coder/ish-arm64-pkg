...
本文用于维护者发布 XCFramework 与匹配的 Corresponding Source。`v0.4.0-abi.12`
已经公开；当前唯一允许准备的下一个标签是兼容性维护版本 `v0.4.0-abi.13`。它仍是 ABI
过渡 prerelease，**不是稳定 v0.4.0**。执行发布会创建公开 GitHub Release 和更新
默认分支，必须在获得明确发布授权后进行。

## `v0.4.0-abi.13` 发布前后状态

### 维护 PR 合入后、Release 发布前

- `Package.swift` 仍固定已发布的 `v0.4.0-abi.12` URL/checksum；
...
- 仓库源码包含已发布的 abi.2 procfs/task 生命周期修复、abi.3 guest `uname`
  ...
  supervisor 延迟 reap，以及待发布 abi.13 的 AArch64 AdvSIMD `REV16` 向量模拟；
- 没有可供使用方安装的 `v0.4.0-abi.13` binary。
...
`--manifest-binary` 证明 Stage1 Swift 仍能链接当前固定的 `v0.4.0-abi.12` binary；
...
除了 SemVer 检查，Stage1 版本策略还会硬性拒绝除 `v0.4.0-abi.13` 以外的任何标签。
因此重用 `v0.4.0-abi.12` 或误输入 `v0.4.0`，都会在任何 tag、draft 或资产写入前失败。
...
只有 `v0.4.0-abi.13` 公开资产、manifest 更新和发布后真链接全部通过，PocketRoot 才能
把依赖从 `v0.4.0-abi.12` 更新到该维护版本并重跑 Xcode 16/iOS 18 门禁。Stage2 与
