...
本文用于维护者发布 XCFramework 与匹配的 Corresponding Source。`v0.4.0-abi.2`
已经公开；当前唯一授权的下一个标签是兼容性维护版本 `v0.4.0-abi.3`。它仍是 ABI
...
- `Package.swift` 仍固定已发布的 `v0.4.0-abi.2` URL/checksum；
- Swift 源保持 v0.3.3 ABI 兼容，不调用 retain/release；
- 仓库源码包含 abi.2 已发布的 procfs/address-space 退出同步与 task 发布锁初始化修复，
  以及 abi.3 新增的 guest `uname` 65 字节字段边界修复；
- 没有可供使用方安装的 `v0.4.0-abi.3` binary。
...
`--manifest-binary` 证明 Stage1 Swift 仍能链接当前固定的 `v0.4.0-abi.2` binary；
...
除了 SemVer 检查，Stage1 版本策略还会硬性拒绝除 `v0.4.0-abi.3` 以外的任何标签。
因此重用 `v0.4.0-abi.2` 或误输入 `v0.4.0`，都会在任何 tag、draft 或资产写入前失败。
...
只有 `v0.4.0-abi.3` 公开资产、manifest 更新和发布后真链接全部通过，PocketRoot 才能
把依赖从 `v0.4.0-abi.2` 更新到该维护版本并重跑 Xcode 16/iOS 18 门禁。Stage2 与
