...
3. **Swift 源与 manifest binary**：Stage1 Swift 保持 v0.3.3 ABI 兼容且不调用
   retain/release。manifest 当前固定已公开的 `v0.4.0-abi.4`；只有
   `v0.4.0-abi.5` 资产公开并验证后，发布事务才将其切到维护 binary。
4. **RootFS 与 PocketRoot**：RootFS 是独立资产；PocketRoot 是上层产品。两者均不因
   runtime PR 或 Release 自动完成。

`v0.4.0-abi.4` 与维护版本 `v0.4.0-abi.5` 都是 native-first 过渡预发布，不是稳定
...
| [发布事务](releasing.md) | 为什么维护源码合入后 manifest 仍是 `v0.4.0-abi.4`？何时变成 `v0.4.0-abi.5`？ |
...
- 当前 `v0.4.0-abi.5` 源码变更不携带 RootFS 或预构建二进制；XCFramework 必须由
  后续发布事务生成。
