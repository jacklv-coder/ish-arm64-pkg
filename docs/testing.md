...
| Swift manifest 真链接 | `test-swift-ios.sh --manifest-binary` | Stage1 Swift 与当前 `v0.4.0-abi.11` binary 链接 | `v0.4.0-abi.12` 修复已公开 |
...
- control queue 的普通/关键 frame/byte 上限、饱和时 close/有限 oneshot 的有界 EOF
  ...
  stdin write/close 复用原始 SPAWN deadline 的回归、有界 stdin-close/terminate、active write
  后返回 busy 的 stdin-close、按调用 write/close deadline 在 legacy session 上仍有界、
  stop/finish 精确释放，以及阻塞 reader 下 spawn staging gate；测试使用较小预算让溢出
  与复用路径可确定复现；
