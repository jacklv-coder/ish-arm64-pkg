...
的 `v0.4.0-abi.10`。发布 `v0.4.0-abi.11` 后，只有 manifest URL/checksum 应切到维护资产。
...
- 已发布的 `v0.4.0-abi.5` 在此基础上增加有限 streaming control-path deadline。
  ...
  竞态；待发布的 `v0.4.0-abi.11` 会等完整 thread group 静止后才允许破坏性 wait 回收；
  这些版本
