...
的 `v0.4.0-abi.12`。发布 `v0.4.0-abi.13` 后，只有 manifest URL/checksum 应切到维护资产。
...
- 已发布的 `v0.4.0-abi.5` 在此基础上增加有限 streaming control-path deadline。
  ...
  `v0.4.0-abi.12` 修复 Apple 写者饥饿并对短暂延迟的 destructive reap 重试；待发布的
  `v0.4.0-abi.13` 补齐 AArch64 AdvSIMD `REV16` 向量模拟；
