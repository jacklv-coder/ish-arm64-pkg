...
的 `v0.4.0-abi.4`。发布 `v0.4.0-abi.5` 后，只有 manifest URL/checksum 应切到维护资产。
...
- 待发布的 `v0.4.0-abi.5` 在此基础上增加有限 streaming control-path deadline；
  它不改变 guest signal 语义。
