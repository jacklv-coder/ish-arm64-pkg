...
的 `v0.4.0-abi.13`。发布 `v0.4.0-abi.14` 后，只有 manifest URL/checksum 应切到维护资产。
...
## guest `uname` 触发 SIGTRAP

症状：guest 启动或 shell 探测系统信息时退出，crash stack 包含
`__chk_fail_overflow`、`__strcpy_chk`、`do_uname` 和 `sys_uname`。这通常由超过
...
- `v0.4.0-abi.4` 已在嵌入启动线程和每个 guest task 线程明确解除内部 SIGUSR1
...
- 已发布的 `v0.4.0-abi.5` 在此基础上增加有限 streaming control-path deadline。
  ...
  的复用；`v0.4.0-abi.7` 新增 guest 原子 no-replace rename；`v0.4.0-abi.8` 让有限
  session 的 stdin write/close 共享同一绝对 deadline；`v0.4.0-abi.9` 新增单次 stdin
  write/close timeout API；已发布的 `v0.4.0-abi.10` 修复强制 task teardown 生命周期
  竞态；已发布的 `v0.4.0-abi.11` 会等完整 thread group 静止后才允许破坏性 wait 回收；
  `v0.4.0-abi.12` 修复 Apple 写者饥饿并对短暂延迟的 destructive reap 重试；已发布的
  `v0.4.0-abi.13` 补齐 AArch64 AdvSIMD `REV16` 向量模拟；待发布的 `v0.4.0-abi.14`
  修正 Darwin/Linux `sockaddr` 布局转换；
  这些版本都不改变 guest signal 语义。
