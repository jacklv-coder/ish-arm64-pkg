...
的 `v0.4.0-abi.5`。发布 `v0.4.0-abi.6` 后，只有 manifest URL/checksum 应切到维护资产。
...
- 已发布的 `v0.4.0-abi.5` 在此基础上增加有限 streaming control-path deadline。
  待发布的 `v0.4.0-abi.6` 补全 Swift 参数封送和 stdin close 对原始 SPAWN deadline
  的复用；两者都不改变 guest signal 语义。
