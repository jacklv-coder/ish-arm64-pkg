...
Corresponding Source. `v0.4.0-abi.2` is already public; the only currently
authorized next tag is the compatible maintenance release `v0.4.0-abi.3`. It
...
## State before and after `v0.4.0-abi.3` publication

### After the maintenance PR merges, before Release publication

- `Package.swift` still pins the published `v0.4.0-abi.2` URL/checksum;
- Swift source remains v0.3.3-ABI compatible and does not call retain/release;
- repository source contains the abi.2 procfs/address-space teardown
  synchronization and task-publication lock-initialization fixes plus the new
  abi.3 65-byte guest `uname` field bound;
- there is no installable `v0.4.0-abi.3` binary.

This intermediate state is intentional: the default branch never advertises an
unpublished asset URL that returns 404.

### After successful `v0.4.0-abi.3` publication
...
`v0.4.0-abi.2` binary. `--local-binary` proves that the same Swift source links
...
```sh
scripts/release.sh v0.4.0-abi.3
```
...
except `v0.4.0-abi.3`. Reusing `v0.4.0-abi.2` or accidentally entering
...
git show v0.4.0-abi.3:Package.swift
...
PocketRoot may move its dependency from `v0.4.0-abi.2` to the maintenance
release only after the public `v0.4.0-abi.3` assets, manifest update, and
