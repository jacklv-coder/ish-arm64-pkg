...
Corresponding Source. `v0.4.0-abi.13` is already public; the only next tag
allowed for preparation is the compatible maintenance release
`v0.4.0-abi.14`. It remains an ABI-transition prerelease that is **not stable
v0.4.0**. Publication
creates a public GitHub Release and updates the default branch, so run it only
with explicit release authorization.

## State before and after `v0.4.0-abi.14` publication

### After the maintenance PR merges, before Release publication

- `Package.swift` still pins the published `v0.4.0-abi.13` URL/checksum;
...
- repository source contains the published abi.2 procfs/task lifecycle fixes,
  ...
  plus deferred supervisor reap, abi.13 AArch64 AdvSIMD vector `REV16`
  emulation, and the pending abi.14 Darwin/Linux `sockaddr` layout translation;
- there is no installable `v0.4.0-abi.14` binary.

This intermediate state is intentional: the default branch never advertises an
unpublished asset URL that returns 404.

### After successful `v0.4.0-abi.14` publication
...
`v0.4.0-abi.13` binary. `--local-binary` proves that the same Swift source links
...
```sh
scripts/release.sh v0.4.0-abi.14
```
...
except `v0.4.0-abi.14`. Reusing `v0.4.0-abi.13` or accidentally entering
...
git show v0.4.0-abi.14:Package.swift
...
PocketRoot may move its dependency from `v0.4.0-abi.13` to the maintenance
release only after the public `v0.4.0-abi.14` assets, manifest update, and
