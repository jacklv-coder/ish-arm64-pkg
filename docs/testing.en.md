...
| --- | --- | --- | --- |
| Protocol unit | `proto_test`, `supervisor_stdin_test` | v4 framing, boundaries, stdin partial writes/backpressure | a real iSH boot |
| Lifecycle | `lifecycle_test` | retain/release, close/read/write/signal interleavings, shutdown/busy, PID identity | Stage2 Swift borrowing is complete |
...
- ordinary/critical control-queue frame/byte ceilings, bounded EOF fallback for
  close and finite oneshot under saturation, exact stop/finish release, and the
...
CI also runs the same suite with `-Db_sanitize=thread` in a separate
`build-ci-tsan` directory. Do not switch sanitizer configurations inside one
build directory; cached Meson state and mixed runtimes make results ambiguous.
Preserve the complete test log and first sanitizer stack on failure.
